...
void analyzeRx()
{
    ...
    {
        case 0:
            measure = rx.dataRx.valueL.b0? 1 : 0;
            break;
        case 1:
...
void transmitTx()
{
    ...
    {
        FVRCON = 0x83;
        tx.lum.fullReg = (uint16_t)((float)ADC_GetConversion(Lum)*lum_gain*0.9765625);
        tx.lum.fullReg = tx.lum.fullReg << 1;
        tx.lum.valueL.fullReg = tx.lum.valueL.fullReg >> 1;
        tx.lum.valueL.b7 = 0;
        FVRCON = 0x82;
        tx.temp.fullReg = (uint16_t)((float)ADC_GetConversion(Temp)*temp_gain*2.0);
